...
def index():
    return render_template('book-recommender-index.html')
    
@app.route('/implicit-goodreads')
def implicit_goodreads():
    return render_template('book-recommender-implicit-goodreads.html')

@app.route('/explicit-goodreads')
def explicit_goodreads():
    return render_template('book-recommender-explicit-goodreads.html')

@app.route('/explicit-ratings')
def explicit_ratings():
    return render_template('book-recommender-explicit-ratings.html')

@app.route('/implicit-recommendations-goodreads', methods = ['GET','POST'])
def implicit_recs_gr():
    if request.method == 'GET':
        ...
        return render_template('implicit_recommendations_goodreads.html', predictions = predictions)

@app.route('/explicit-recommendations-goodreads', methods=['GET','POST'])
def explicit_recs_gr():
    if request.method == 'GET':
        user_id = request.args['goodreads-id']
        print(f'goodreads ID: {user_id}')
        sys.stdout.flush()
        ...
        print('Now requesting user reviews from Goodreads')
        sys.stdout.flush()
        reviews_df = get_reviews(api_key, user_id, books, dataset_explicit)

        weights_array = get_user_array(reviews_df)
        ...
        return render_template('explicit_recommendations_goodreads.html', predictions = predictions)
