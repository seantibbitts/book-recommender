...
def explicit_recs():
    if request.method == 'GET':
        ...
        return render_template('explicit_recommendations.html', predictions = predictions)
    
    if request.method == 'POST':
        start_time = time.time()
        weights_array = np.array(json.loads(str(request.json))).reshape(1, -1)
        print('Loaded array')
        sys.stdout.flush()

        assert weights_array.shape[1] == weights_explicit.shape[1]
        # Causes system to run out of memory
        predictions = predict_for_user_knn_lightfm(model_explicit, dataset_explicit, weights_explicit,
        books, weights_array, item_features=item_features, num_recs = 24).to_dict(orient='records')
